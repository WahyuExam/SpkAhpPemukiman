unit u_lihat_hitung_sub_kriteria;
...
type
  Tf_lihat_hitung_sub_kriteria = class(TForm)
    grp6: TGroupBox;
    lbl5: TLabel;
    lbl6: TLabel;
    lbl7: TLabel;
...
var
  f_lihat_hitung_sub_kriteria: Tf_lihat_hitung_sub_kriteria;
...
procedure Tf_lihat_hitung_sub_kriteria.btn1Click(Sender: TObject);
begin
 close;
end;

procedure Tf_lihat_hitung_sub_kriteria.btn2Click(Sender: TObject);
...
begin
  with dm.qry_sub_kriteria do
   begin
     close;
     SQL.Clear;
     SQL.Text:='select * from tbl_sub_kriteria where kd_kriteria='+QuotedStr(edtkd_kriteria.Text)+' order by kd_sub_kriteria asc';
     Open;
     ...
     for a:=1 to RecordCount do
      begin
        ...
        with StringGrid1 do
         begin
           ...
           Cells[0,0+a] := fieldbyname('kd_sub_kriteria').AsString;
           Cells[0+a,0] := fieldbyname('kd_sub_kriteria').AsString;
           Cells[a,a] := '1';
         end;
      ...
      StringGrid4.Cells[0,0] := 'Subkriteria';
  ...
  with dm.qry_sub_kriteria do
   begin
     ...
     for a:=1 to RecordCount do
      begin
        RecNo := a;
        kode_kriteria := fieldbyname('kd_sub_kriteria').AsString;

        with dm.qry_tmp_sub_kriteria do
         begin
           close;
           SQL.Clear;
           sql.Text:='select * from tbl_tmp_sub_kriteria where kd_kriteria='+QuotedStr(edtkd_kriteria.Text)+' and kd_sub_kriteria1='+QuotedStr(kode_kriteria)+'';
           Open;

           for b:=1 to RecordCount do
            begin
              RecNo:=b;
              kriteria1 := fieldbyname('kd_sub_kriteria1').AsString;
              kriteria2 := fieldbyname('kd_sub_kriteria2').AsString;
              hasil_bagi := 1/fieldbyname('nil').AsVariant;

              with dm.qry_tmp_sub_kriteria do
                begin
                 if Locate('kd_sub_kriteria1;kd_sub_kriteria2;nil',VarArrayOf([kriteria1,kriteria2,fieldbyname('nil').AsString]),[]) then
                  begin
                    Edit;
    ...
    with StringGrid3 do
     ...
     with StringGrid4 do
      ...
      if dm.qry_indek.Locate('matrik',IntToStr(StringGrid1.ColCount-1),[]) then
       begin
         if dm.qry_indek.FieldByName('nilai_ir').AsVariant = 0.00 then
          begin
            rasio_konsistensi := StrToFloat(lbl9.Caption);
          end
          else
          begin
                ...
                begin
                  with dm.qry_sub_kriteria do
                   begin
                     ...
                     RecNo:=a;
                     Edit;
                     FieldByName('nl_sub').AsString := StringGrid2.Cells[colcount-1,a];
                     Post
                   end;
                end;
                a:=a+1;
   ...
   with dm.qry_tampil_sub_kriteria do
    begin
      ...
      sql.Add('tbl_kriteria b where a.kd_kriteria=b.kd_kriteria and b.kd_kriteria='+QuotedStr(edtkd_kriteria.Text)+' order by a.kd_sub_kriteria asc');
      Open;
    end;
  ...
  with dm.qry_tmp_sub_kriteria do
   begin
     close;
     SQL.Clear;
     SQL.Text:='select * from tbl_tmp_sub_kriteria where kd_kriteria='+QuotedStr(edtkd_kriteria.Text)+' order by kd_sub_kriteria1 asc, kd_sub_kriteria2 asc';
     Open;
   end;
end;
